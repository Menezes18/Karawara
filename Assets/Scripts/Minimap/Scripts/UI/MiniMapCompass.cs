...
namespace System.MiniMap
{
    public class MiniMapCompass : MonoBehaviour
    {
        ...
        void Start()
        {
            MiniMap = GetComponentInParent<SystemMiniMap>();
            if (Target == null)
            {
        ...
        void Update()
        {
            ...
            if (MiniMap.mapShape == SystemMiniMap.MapShape.Circle)
            {
                if (MiniMapUtils.RenderCamera == null) return;

                Vector3 north = Vector3.forward * 1000;
                Vector3 tar = MiniMapUtils.RenderCamera.transform.forward;
                tar.y = 0;
