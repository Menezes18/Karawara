...
#if UNITY_EDITOR
    [CustomEditor(typeof(MiniMapRenderTool))]
    public class bl_MiniMapScreenShotEditor : Editor
    {
        MiniMapRenderTool script;
        Camera m_Camera;
        ...
        {
            script = (MiniMapRenderTool)target;
            m_Camera = script.GetComponent<Camera>();

            if (!init)
            {
                if (mapBounds == null && FindObjectOfType<MiniMapBounds>() != null)
                {
                    mapBounds = FindObjectOfType<MiniMapBounds>();
                }
        ...
        public override void OnInspectorGUI()
        {
            ...
            script.renderDivisions = (SystemMapRender.RendersDivisions)EditorGUILayout.EnumPopup("Render Divisions", script.renderDivisions);
            ...
            mapBounds = EditorGUILayout.ObjectField("Map Bounds", mapBounds, typeof(MiniMapBounds), true) as MiniMapBounds;
        ...
        public void TakeSnapshot()
        {
            ...
            var renderContainer = ScriptableObject.CreateInstance<SystemMapRender>();
            renderContainer.renderDivisions = script.renderDivisions;
            renderContainer.snapshots = new List<Texture2D>();

            if (script.renderDivisions == SystemMapRender.RendersDivisions.Single)
            {
        ...
        /// <param name="path"></param>
        /// <returns></returns>
        private SystemMapRender SaveRenderContainer(SystemMapRender instance, string path)
        {
            AssetDatabase.CreateAsset(instance, path);
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();

            var container = AssetDatabase.LoadAssetAtPath<SystemMapRender>(path);
            return container;
        }
        ...
        private void OnSceneGUI()
        {
            ...
            if (script.renderDivisions == SystemMapRender.RendersDivisions.Single) return;
